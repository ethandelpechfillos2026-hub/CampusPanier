...
export function Placeholder({ children }: { children: ReactNode }) {
  return (
    <span className="rounded bg-yellow-200 px-1.5 py-0.5 font-semibold text-yellow-900">
...
export function DecisionNeeded({ children }: { children: ReactNode }) {
  return (
    <span className="rounded bg-blue-100 px-1.5 py-0.5 font-semibold text-blue-900">
      [DÉCISION À PRENDRE — {children}]
    </span>
  );
}
...
export function LawyerCheck({ children }: { children: ReactNode }) {
  return (
    <span className="rounded bg-red-100 px-1.5 py-0.5 font-semibold text-red-900">
      [À VÉRIFIER PAR UN JURISTE — {children}]
    </span>
  );
}
